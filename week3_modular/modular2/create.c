...
int create_init(SharedVariable* sv) {
...
void forward(SharedVariable* sv) {
...
void reverse(SharedVariable* sv) {
char reverse[] = {137, 255, 100, 128, 0 };
int wd;
if(sv->current_direction != REVERSE) {
wd =  write(fd, reverse, 8);
printf("Direction changed to reverse\n");
}
sv->current_direction = REVERSE;
}

void left(SharedVariable* sv) {
char left[] = {137, 0, 100, 0, 1 };
int wd;
if(sv->current_direction != LEFT) {
wd =  write(fd, left, 8);
printf("Direction changed to left");
}
sv->current_direction = LEFT;
}

void right(SharedVariable* sv) {
char right[] = {137, 0, 100, 255, 255 }; 
int wd;
if(sv->current_direction != RIGHT){
wd =  write(fd, right, 8);
printf("Direction changed to right");
}
sv->current_direction = RIGHT;
}
